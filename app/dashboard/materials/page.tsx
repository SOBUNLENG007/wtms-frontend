...
export default function MaterialsPage() {
  ...
  const filtered = materials.filter((m) => {
    const matchesSearch = m.title.toLowerCase().includes(search.toLowerCase()) || m.sessionTitle.toLowerCase().includes(search.toLowerCase())
  ...
  function handleUpload(e: React.FormEvent<HTMLFormElement>) {
    ...
    const newMaterial: Material = {
      ...
      type: (fd.get("type") as Material["type"]) || "pdf",
      sessionId: "s1",
      sessionTitle: fd.get("session") as string || "Cybersecurity Fundamentals",
      uploadedBy: user?.name || "",
      uploadedAt: new Date().toISOString().split("T")[0],
      size: "1.2 MB",
      accessLevel: (fd.get("access") as Material["accessLevel"]) || "all",
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {isTrainerOrAdmin ? "Upload, organize, and manage access to training materials." : "Browse and download training materials."}
          </p>
        </div>
        {isTrainerOrAdmin && (
      ...
      {Object.entries(grouped).map(([session, mats]) => (
        <div key={session} className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary" className="text-[10px]">{mats.length} files</Badge>
          </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-muted-foreground mt-2">
                          By {material.uploadedBy} &middot; {new Date(material.uploadedAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                        </p>
                      </div>
                    </div>
                  </CardContent>
      ...
      <Dialog open={showUpload} onOpenChange={setShowUpload}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Upload documents, slides, or videos for training sessions.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleUpload} className="space-y-4 mt-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                ...
                <Select name="access" defaultValue="all">
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">All Users</SelectItem>
                    <SelectItem value="department">Department Only</SelectItem>
                    <SelectItem value="specific">Authorized Only</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <div className="rounded-lg border-2 border-dashed border-border p-6 text-center">
              ...
              <p className="text-xs text-muted-foreground/60 mt-1">PDF, PPTX, MP4 up to 500MB</p>
            </div>
